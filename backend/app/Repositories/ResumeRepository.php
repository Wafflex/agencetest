...
class ResumeRepository extends Repository
{
    ...
    public function getResume($users, $interval){
        ...
        return DB::table('cao_fatura as factura')
                    ->selectRaw('os.co_usuario, usuario.no_usuario, MONTH(factura.data_emissao) as mes, YEAR(factura.data_emissao) as anio, ROUND(SUM(factura.valor - (factura.total_imp_inc/100)),2) as neto, ROUND(SUM(factura.valor - (factura.valor * factura.total_imp_inc/100) * factura.comissao_cn),2) as comision, cao_salario.brut_salario as salario')
